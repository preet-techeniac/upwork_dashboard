...
export default function EditBidPage() {
  ...
  useEffect(() => {
    if (!token) return;
    authFetch(`/api/bids/${id}`, token).then((r) => r.json()).then((bidRes) => {
      const b = bidRes.data;
      if (b) {
        let cvName = b.cv_used ?? '';
        let cvUrl = '';
        let propName = b.proposal_used ?? '';
        let propUrl = '';

        const cvMatch = cvName.match(/^\[(.*?)\]\((.*?)\)$/);
        if (cvMatch) { cvName = cvMatch[1]; cvUrl = cvMatch[2]; }

        const propMatch = propName.match(/^\[(.*?)\]\((.*?)\)$/);
        if (propMatch) { propName = propMatch[1]; propUrl = propMatch[2]; }

        setForm({
          application_date: b.application_date ? new Date(b.application_date).toISOString().slice(0, 10) : '',
          job_url: b.job_url ?? '',
          found_by: b.found_by ?? '',
          cv_name: cvName,
          cv_url: cvUrl,
          proposal_name: propName,
          proposal_url: propUrl,
          status: b.status ?? '',
          hires: b.hires ?? '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await authFetch(`/api/bids/${id}`, token, {
        method: 'PUT',
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
      <Topbar title="Edit Bid" subtitle="Update bid details" />
      <main style={{ flex: 1, padding: '1.5rem', maxWidth: 640, overflowY: 'auto' }}>
        <div style={{ marginBottom: '1.5rem' }}>
          ...
          <h2 className="page-title">Edit Bid</h2>
        </div>
